-- =============================================================
-- ToeflMaster ITP — Full Schema Migration
-- Run this in Supabase SQL Editor (Dashboard → SQL Editor → New query)
-- Based on Implementation Plan Section 4
-- =============================================================
...
-- ============================================
-- READING PASSAGES (must exist before questions)
-- ============================================
...
CREATE TABLE IF NOT EXISTS questions (
  ...
  -- Section 1: 'A'|'B'|'C'  Section 2: 'structure'|'written_expression'
  part                     TEXT,
  ...
  audio_url                TEXT,
  -- Groups questions belonging to same audio clip (Part B / Part C)
...
CREATE TABLE IF NOT EXISTS test_sessions (
  ...
  -- Section timing (seconds remaining when saved)
  s1_time_remaining     INTEGER DEFAULT 2100,  -- 35 min
  s2_time_remaining     INTEGER DEFAULT 1500,  -- 25 min
  s3_time_remaining     INTEGER DEFAULT 3300,  -- 55 min

  -- Raw scores (correct answers count)
  s1_raw_score          INTEGER DEFAULT 0,
  s2_raw_score          INTEGER DEFAULT 0,
  s3_raw_score          INTEGER DEFAULT 0,

  -- Scaled scores (computed after completion)
  s1_scaled_score       INTEGER,
  s2_scaled_score       INTEGER,
  s3_scaled_score       INTEGER,
  total_scaled_score    INTEGER,

  -- Question order (shuffled array of Question IDs)
...
-- ============================================
-- PROGRESS TRACKING (Aggregated, daily)
-- ============================================
CREATE TABLE IF NOT EXISTS user_progress (
  ...
  -- Sub-skill accuracy (0.00–100.00)
...
-- ============================================
-- USER VOCABULARY (personalized SRS)
-- ============================================
...
-- Public read for question bank and passages (questions are shared)
...
-- USER VOCABULARY
-- -------------------------------------------------------
CREATE POLICY "user_vocab_all_own"
  ON user_vocabulary FOR ALL
  USING (auth.uid() = user_id);

-- -------------------------------------------------------
-- QUESTIONS & PASSAGES (Public read — shared content)
-- -------------------------------------------------------
CREATE POLICY "questions_public_read"
  ON questions FOR SELECT
  USING (is_active = TRUE);

CREATE POLICY "passages_public_read"
...
CREATE POLICY "audio_public_read"
  ON audio_conversations FOR SELECT
  USING (is_active = TRUE);
...
-- Score equating approximation (raw → scaled)
-- Section 1 & 2: scale 31–68 | Section 3: scale 31–67
...
BEGIN
  ...
  IF section_num IN (1, 2) THEN
    scaled := ROUND(31 + (percentage * 37));  -- 31–68
  ELSE
    scaled := ROUND(31 + (percentage * 36));  -- 31–67
  END IF;
...
-- Study streak update function
-- Call this whenever user_progress is upserted
...
BEGIN
  ...
  IF v_last_date IS NULL OR v_last_date < v_today - INTERVAL '1 day' THEN
    ...
    IF v_last_date < v_today - INTERVAL '1 day' OR v_last_date IS NULL THEN
      v_current := 1;
    END IF;
  ...
  END IF;
  -- today already counted — no change if last_study_date = today
  ...
  -- Sync to profiles
  UPDATE profiles SET study_streak = v_current
  WHERE id = p_user_id;
END;
$$;
